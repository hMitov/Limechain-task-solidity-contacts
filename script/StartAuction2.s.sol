...
contract StartAuction2Script is Script {
    ...
    function run() public {
        uint256 privateKey;
        try vm.envBytes32("TEST_ACCOUNT_2_PRIVATE_KEY") returns (bytes32 keyBytes) {
            require(keyBytes != bytes32(0), "TEST_ACCOUNT_2_PRIVATE_KEY is empty");
            privateKey = uint256(keyBytes);
        } catch {
            revert("TEST_ACCOUNT_2_PRIVATE_KEY is missing in .env");
        }

        address auctionAddress;
        try vm.envAddress("AUCTION_2_ADDRESS") returns (address addr) {
            require(addr != address(0), "AUCTION_2_ADDRESS is zero address");
            auctionAddress = addr;
        } catch {
            revert("AUCTION_2_ADDRESS is missing or invalid in .env");
        }
